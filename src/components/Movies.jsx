...
function Movies() {
    ...
    // axios.get("https://api.themoviedb.org/3/trending/all/week?api_key=57809852304910d4194f33adc8e13c5b").then((res) => {
    ...
    <div className="mt-8">
      ...
      /> :
          movies.map((movie) => {
           ...
           key = {movie.id} 
         className= "bg-center bg-cover  w-[160px] h-[30vh] md:h-[40vh] md:w-[180px] m-4 rounded-xl hover:scale-110 duration-300 border-4 flex items-end" 
         style={{
                                          backgroundImage:
                                              `url(
          ...
          <div className=" p-2 bg-gray-800 absolute top-2 right-2 " style = {{
            display:hovered == movie.id ? "block":"none" 
          }}
          >
          
          {favourites.includes(movie.id) == false ? <div className="text=2xl"
